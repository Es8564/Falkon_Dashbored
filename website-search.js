...
(function() {
    ...
    var INDEX = [
        ...
        { url: 'docs.html#webrequest', title: 'Enable WebRequest (MANDATORY)', category: 'Setup', keywords: 'webrequest allow url enable permissions mt5 tools options expert advisors script.google.com mandatory critical connection', description: 'MANDATORY: MT5 → Tools → Options → Expert Advisors → tick "Allow WebRequest for listed URL" → add https://script.google.com/ — without this the EA cannot connect.' },
        { url: 'setup.html#credentials', title: 'Where to Paste URL & Token', category: 'Setup', keywords: 'where paste url token RC_EndpointURL RC_SecretToken google apps script exec unique token per account inputs settings remote control', description: 'In EA Inputs tab → "Google Apps Script /exec URL" (RC_EndpointURL) = paste server URL. "UNIQUE token per account" (RC_SecretToken) = paste your token from welcome email or Account page.' },
